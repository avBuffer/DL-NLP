...
# -*- coding: utf-8 -*-
import struct  
from pyparsing import unichr, unicode
...
def byte2str(data):
    ...
    ret = u''  
    while i < length:  
        x = (data[i] + data[i + 1]).to_bytes(length=2, byteorder='big', signed=True)
        print('x=', x)
        print('struct.unpack=', struct.unpack('H', x))
        print('struct.unpack0=', struct.unpack('H', x)[0])
...
def getWordPy(data):  
    ...
    ret = u''  
    while pos < length:
        x = (data[pos] + data[pos + 1]).to_bytes(length=2, byteorder='big', signed=True)
        index = struct.unpack('H', x)[0]
        print('index=', index, 'GPy_Table=', GPy_Table)
        ret += GPy_Table[index]  
        pos += 2      
    return ret  
...
def getChinese(data):
    ...
    while pos < length:
        x = (data[pos] + data[pos + 1]).to_bytes(length=2, byteorder='big', signed=True)
        #同音词数量  
        same = struct.unpack('H', x)[0]
        #拼音索引表长度  
        pos += 2  
        py_table_len = struct.unpack('H', x)[0]
        #拼音索引表  
        pos += 2  
