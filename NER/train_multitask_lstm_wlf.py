...
def valid(data_processor, max_batches=None, batch_size=1024):
    ...
    while True:
        inputs_seq_char_batch, inputs_seq_word_batch, inputs_seq_len_batch, \
        outputs_seq_bio_batch, outputs_seq_attr_batch = data_processor.get_batch(batch_size)

        feed_dict = {model.inputs_seq_char: inputs_seq_char_batch, model.inputs_seq_word: inputs_seq_word_batch,
                     model.inputs_seq_len: inputs_seq_len_batch, model.outputs_seq_bio: outputs_seq_bio_batch,
        ...
        for pred_seq_bio, gold_seq_bio, pred_seq_attr, gold_seq_attr, input_seq, l in zip(preds_seq_bio_batch, outputs_seq_bio_batch,
                                                                                          ...
                                                                                          inputs_seq_char_batch, inputs_seq_len_batch):
...
if __name__ == '__main__':
    ...
    # set logging
    log_file_path = os.path.join(ckpt_path, "train_multitask_lstm_wlf_run_log.txt")
    ...
    data_processor_train = DataProcessor("data/train/input.seq.char", "data/train/input.seq.word",
                                         "data/train/output.seq.bio", "data/train/output.seq.attr",
                                          w2i_char, w2i_word, w2i_bio, w2i_attr, shuffling=True)
    data_processor_valid = DataProcessor("data/test/input.seq.char", "data/test/input.seq.word",
                                         "data/test/output.seq.bio", "data/test/output.seq.attr",
                                          w2i_char, w2i_word, w2i_bio, w2i_attr, shuffling=True)

    logger.info("building model...")
    model = MyModel(embedding_dim=300, hidden_dim=300, vocab_size_char=len(w2i_char), vocab_size_word=len(w2i_word),
                    vocab_size_bio=len(w2i_bio), vocab_size_attr=len(w2i_attr), O_tag_index=w2i_bio["O"], use_crf=False)
    ...
    with tf.Session(config=config) as sess:
        ...
        while epoches < 20:
            inputs_seq_char_batch, inputs_seq_word_batch, inputs_seq_len_batch, \
            outputs_seq_bio_batch, outputs_seq_attr_batch = data_processor_train.get_batch(batch_size)
            ...
            if batches % 100 == 0:
                ...
                ckpt_save_path = os.path.join(ckpt_path, "train_multitask_lstm_wlf-model-{}_{}".format(batches, ave_loss))
