...
class DataProcessor_BERT(object):
    ...
    def get_batch(self, batch_size):
        ...
        for input_seq, input_mask, input_segment, output_seq, l in zip(inputs_seq_batch, inputs_mask_batch,
                                                                       inputs_segment_batch, outputs_seq_batch, lens):
        ...
        return (np.array(inputs_seq_batch, dtype="int32"), np.array(inputs_mask_batch, dtype="int32"),
                np.array(inputs_segment_batch, dtype="int32"), np.array(outputs_seq_batch, dtype="int32"))


####### DataProcessor 3: for Multitask-LSTM #######
class DataProcessor_MTL_LSTM(object):
    def __init__(self, input_seq_path, output_seq_bio_path, output_seq_attr_path, w2i_char, w2i_bio, w2i_attr, shuffling=False):
        inputs_seq = []
        with open(input_seq_path, "r", encoding="utf-8") as f:
            for line in f.read().strip().split("\n"):
                seq = [w2i_char[word] if word in w2i_char else w2i_char["[UNK]"] for word in line.split(" ")]
    ...
    def get_batch(self, batch_size):
        ...
        return (np.array(inputs_seq_batch, dtype="int32"), np.array(inputs_seq_len_batch, dtype="int32"),
                np.array(outputs_seq_bio_batch, dtype="int32"), np.array(outputs_seq_attr_batch, dtype="int32"))
...
class DataProcessor_MTL_BERT(object):
    ...
    def get_batch(self, batch_size):
        ...
        max_seq_len = max(lens)
        for input_seq, input_mask, input_segment, output_seq_bio, output_seq_attr, l in zip(inputs_seq_batch, inputs_mask_batch,
                                                                                            inputs_segment_batch, outputs_seq_bio_batch,
                                                                                            outputs_seq_attr_batch, lens):
            input_seq.extend([self.w2i_char["[PAD]"]] * (max_seq_len - l))
            input_mask.extend([0] * (max_seq_len - l))
            input_segment.extend([0] * (max_seq_len - l))
...
class DataProcessor_MTL_LSTM_WLF(object):
    def __init__(self, input_seq_char_path, input_seq_word_path, output_seq_bio_path, output_seq_attr_path,
                 w2i_char, w2i_word, w2i_bio, w2i_attr, shuffling=False):
        inputs_seq_char = []
