...
class PoswiseFeedForwardNet(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Sequential(nn.Linear(d_model, d_ff, bias=False), nn.ReLU(), nn.Linear(d_ff, d_model, bias=False))
...
def greedy_decoder(model, enc_input, start_symbol):
    """For simplicity, a Greedy Decoder is Beam search when K=1. This is necessary for inference as we don't know the target sequence input.
...
if __name__ == '__main__':
    ...
    for epoch in range(max_iter):
        for idx, (enc_inputs, dec_inputs, dec_outputs) in enumerate(loader):
            ''' enc_inputs: [batch_size, src_len]
                dec_inputs: [batch_size, tgt_len]
            ...
            if epoch % save_interval == 0:
                model_file = os.path.join(ckpt_path, "transformer_{}-{}.th".format(epoch, loss.item()))
